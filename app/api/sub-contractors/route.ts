...
export async function GET(req: Request) {
  try {
    ...
    if (error) {
      console.error("[sub-contractors]", error);
      return jsonError(500, { error: "Failed to load sub contractors" });
    }
    ...
    console.error("[sub-contractors]", e);
...
export async function POST(req: Request) {
  try {
    ...
    if (error) {
      console.error("[sub-contractors]", error);
      return jsonError(500, { error: "Failed to create sub contractor" });
    }
