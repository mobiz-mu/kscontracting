...
export default function CreditNotePrintPage() {
  ...
  const load = React.useCallback(async () => {
    if (!hasId) {
      setError("Missing credit note id");
      setCreditNote(null);
      return;
    }
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {creditNote ? (
                <Chip className="bg-slate-50 text-slate-700 ring-slate-200">
                  ...
                  {creditNote.credit_note_no || creditNote.note_no || "—"}
                </Chip>
              ) : null}
            </div>
            ...
            <div className="mt-3">
              <h1 className="text-xl font-extrabold tracking-tight text-slate-950 sm:text-2xl">
                Premium Credit Note Print View
              </h1>
              <p className="mt-1 text-sm text-slate-600">
                {creditNote
                  ? "Credit note ready for printing or PDF export."
                  : loading
                  ? "Loading credit note document..."
                  : hasId
                  ? "Credit note preview unavailable."
                  : "Missing credit note id."}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href={
                hasId
                  ? `/sales/credit-notes/${encodeURIComponent(id)}`
                  : "/sales/credit-notes"
              }
            >
              <Button
      ...
      <div className="mt-3 text-xs text-slate-400 print:hidden">
        {creditNote ? (
          <>
            ...
            • Items:{" "}
            <span className="font-semibold">{creditNote.items?.length ?? 0}</span>
          </>
        ) : (
          "—"
        )}
