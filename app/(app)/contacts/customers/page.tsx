...
export default function CustomersPage() {
  ...
  async function load(targetPage = page) {
    try {
      ...
      setError(getErrorMessage(e, "Failed to load customers"));
      setRows([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="relative px-4 py-5 sm:px-6 sm:py-6 xl:px-7 xl:py-7">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <Chip className="bg-white/12 text-white ring-white/15">
                  <Users className="size-3.5 text-white/85" />
                  Contacts
                </Chip>
              ...
              <p className="mt-2 max-w-4xl text-sm leading-6 text-blue-50/90 sm:text-[15px]">
                Premium customer workspace for KS Contracting with quick search,
                clean contact visibility, and direct edit access for each
                customer record.
              </p>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative w-full max-w-[520px]">
              ...
              <Input
                placeholder="Search customer name, email, phone, VAT, BRN..."
              ...
              {loading ? (
                ...
                rows.map((c) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-slate-700">{c.vat_no ?? "—"}</td>

                    <td className="p-4 text-slate-700">{c.brn ?? "—"}</td>

                    <td className="p-4 text-slate-700">{fmtDate(c.created_at)}</td>

                    <td className="p-4">
                      <div className="flex justify-end gap-2">
                        <Link href={`/contacts/customers/${c.id}`}>
                          <Button
                            variant="outline"
