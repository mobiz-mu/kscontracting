...
export default function ForgotPasswordPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // exists, so this form can't be used to enumerate registered users.
      setSent(true);
    } catch (e: unknown) {
      setError(getErrorMessage(e, "Failed to send reset email. Please try again."));
    } finally {
      setLoading(false);
    }
  }
                  ...
                  <div className="space-y-5 text-center">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-600">
                        ...
                        a password reset link is on its way. The link expires
                        after a short time, so use it soon.
                      </p>
                    </div>
                    <Link
                      href="/login"
                      className="inline-flex items-center justify-center gap-2 text-sm font-semibold text-[#ff7a18] hover:text-[#ff6a00]"
                    >
